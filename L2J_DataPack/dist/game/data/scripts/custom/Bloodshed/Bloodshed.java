...
public class Bloodshed extends Quest
{
	...
	// Items
	private final int E_APIGA = Config.SOLOINSTANCE_ITEMID1;
	private final int STONE = Config.SOLOINSTANCE_ITEMID2;
	private final int SCROLLW = Config.SOLOINSTANCE_ITEMID3;
	private final int SCROLLA = Config.SOLOINSTANCE_ITEMID4;
	
	// Npcs
	private final int ROSE = 999026;
	private final int CHEST = 999027;
	
	// Monsters
	private final int NAGLFAR = 199032;
	private final int SENTRY1 = 199033;
	private final int SENTRY2 = 199034;
	private final int HOUND = 199035;
	
	// Miscs
	private final int TIMELIMIT = 36000;
	private final int INSTANCEID = 5000;
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		{
			player.sendMessage(Config.CUSTOM_MSG_1001);
			return false;
		}
	...
	protected void exitInstance(L2PcInstance player, teleCoord tele, int tId)
	{
		...
		player.teleToLocation(Config.SOLOINSTANCE_EXITING_LOC_X, Config.SOLOINSTANCE_EXITING_LOC_Y, Config.SOLOINSTANCE_EXITING_LOC_Z);
		final L2Summon pet = player.getSummon();
		if (pet != null)
		{
			pet.setInstanceId(0);
			pet.teleToLocation(Config.SOLOINSTANCE_EXITING_LOC_X, Config.SOLOINSTANCE_EXITING_LOC_Y, Config.SOLOINSTANCE_EXITING_LOC_Z);
		}
	}
	...
	public final String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		{
			InstanceWorld world = InstanceManager.getInstance().getPlayerWorld(player);
			player.sendPacket(new ExShowScreenMessage(Config.CUSTOM_MSG_1002, 8000));
			teleCoord tele = new teleCoord();
			tele.instanceId = 0;
			tele.x = 83279;
			tele.y = 148011;
			tele.z = -3404;
			player.addItem("Custom", 57, 1000000, player, true);
			if (Rnd.get(100) < 10)
			{
				player.addItem("Custom", STONE, Config.SOLOINSTANCE_ITEMCNT2, player, true);
			}
			if (Rnd.get(100) < 15)
			{
				player.addItem("Custom", SCROLLW, Config.SOLOINSTANCE_ITEMCNT3, player, true);
			}
			if (Rnd.get(100) < 15)
			{
				player.addItem("Custom", SCROLLA, Config.SOLOINSTANCE_ITEMCNT4, player, true);
			}
			else
			{
	...
	public final String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		switch (npcId)
		{
			...
			case SENTRY2:
				...
				player.sendPacket(new CreatureSay(npc.getObjectId(), 0, npc.getName(), Config.CUSTOM_MSG_1003));
				player.addItem("Solo Instance Event", E_APIGA, Config.SOLOINSTANCE_ITEMCNT1, npc, true);
				openDoor(DOOR2, player.getInstanceId());
				break;
			case HOUND:
				player.sendPacket(new PlaySound("ItemSound.quest_middle"));
				player.sendPacket(new ExShowScreenMessage(Config.CUSTOM_MSG_1004, 8000));
				...
				break;
			case NAGLFAR:
				player.sendPacket(new CreatureSay(npc.getObjectId(), 0, npc.getName(), Config.CUSTOM_MSG_1005));
				player.sendPacket(new ExShowScreenMessage(Config.CUSTOM_MSG_1006, 12000));
	...
	{
		new Bloodshed(-1, qn, "Custom");
		_log.info("Bloodshed Solo Instance Event Loaded.");
	}
}