...
public final class AdminPrimePoints implements IAdminCommandHandler
{
		...
		if (actualCommand.equals("admin_primepoints"))
		{
			if (st.hasMoreTokens())
			{
				...
				switch (action)
				{
					...
					case "increase":
					{
						if (target.getGamePoints() == Integer.MAX_VALUE)
						{
							showMenuHtml(activeChar);
						...
						long primeCount = Math.min((target.getGamePoints() + value), Integer.MAX_VALUE);
						if (primeCount < 0)
						{
							primeCount = Integer.MAX_VALUE;
						}
						target.setGamePoints(primeCount);
					...
					case "decrease":
					{
						...
						final long primeCount = Math.max(target.getGamePoints() - value, 0);
						target.setGamePoints(primeCount);
	...
	private int increaseForAll(Collection<L2PcInstance> playerList, long value)
	{
		int counter = 0;
		for (L2PcInstance temp : playerList)
		{
			...
			{
				if (temp.getGamePoints() == Integer.MAX_VALUE)
				{
					continue;
				}
				
				long primeCount = Math.min((temp.getGamePoints() + value), Integer.MAX_VALUE);
				if (primeCount < 0)
				{
					primeCount = Integer.MAX_VALUE;
				}
				temp.setGamePoints(primeCount);
	...
	private void showMenuHtml(L2PcInstance activeChar)
	{
		...
		final L2PcInstance target = getTarget(activeChar);
		final long points = target.getGamePoints();
